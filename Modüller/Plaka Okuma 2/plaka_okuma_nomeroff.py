...
def get_pipeline():
    ...
    if _n_pipeline is None:
        print("Nomeroff-Net Pipeline yükleniyor (ilk çalıştırmada model indirilebilir)...")
        # Aktif cihazı al
...
def detect_and_read_nomeroff(image_path, run_plates=True):
    """
    Hybrid recognition: 
    - Vehicles via YOLOv8 (Blue boxes)
    - Plates via Nomeroff-Net (Green boxes)
