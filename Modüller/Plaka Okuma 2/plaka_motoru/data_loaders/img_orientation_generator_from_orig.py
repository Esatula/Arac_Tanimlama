...
python3 -m plaka_motoru.data_loaders.img_orientation_generator -f plaka_motoru/data_loaders/img_orientation_generator_from_orig.py
"""
import os
import json
import cv2
...
from plaka_motoru.tools.image_processing import normalize_img, build_perspective
from plaka_motoru.pipes.number_plate_keypoints_detectors.bbox_np_points_tools import normalize_rect


def rotate_rect(rect, angle):
    """
    Povertaє rect kesilen işler için (90, 180 veya 270 derece)ів)
    """
    if angle not in [0, 90, 180, 270]:
        raise ValueError("Kut suçlu ama 0, 90, 180 veya 270 dereceів")
    rect = normalize_rect(rect)

    # Mektupєgüç için mo rect numpy dizisiі
    rect = np.array(rect)

    if angle == 0:
        return [rect[1], rect[2], rect[3], rect[0]]
    elif angle == 90:
        # 90 derecelik bir dönüş için ZsuvіGodinnikov'un sayfası içinіparlak bir şekilde
        return [rect[2], rect[3], rect[0], rect[1]]
    elif angle == 180:
        # 180 derece döndürmek için Zsuvів
        return [rect[3], rect[0], rect[1], rect[2]]
    elif angle == 270:
        # 270 derece döndürmek için ZsuvіGodinnikov'un sayfası içinіlkoyu (90, Godinnikovo'ya karşı)ї)
        return rect
