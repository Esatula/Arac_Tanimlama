...
from plaka_motoru.tools import (fline,
                                distance,
...
def check_line_side_faces(target_points: np.ndarray, shape: Tuple) -> Tuple[List, Dict]:
    """
    Kırık fiş
    yan yüzlerin eğim açısını sabitlemelidir.
    """
    w = shape[1]
...
def get_center_point(p0: List, p1: List) -> List[float]:
    """
    2 noktayla tanımlanan bir segmentin ortasını arıyoruz
    :param p0:
    :param p1:
    :return:
    """
...
def fit_to_frame(target_points: np.ndarray or List, mline_boxes: List,  shape: Tuple) -> Tuple[List, List]:
    """
    Kırık fiş
    metin alanını belirli bir çerçeveye sığdıracak olan
    """
    h = shape[0]
    w = shape[1]
