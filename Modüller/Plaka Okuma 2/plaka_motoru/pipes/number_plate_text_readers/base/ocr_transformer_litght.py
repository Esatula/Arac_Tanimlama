"""
python3.9 -m plaka_motoru.pipes.number_plate_text_readers.base.ocr_transformer_litght -f ./plaka_motoru/pipes/number_plate_text_readers/base/ocr_transformer_litght.py
"""
import os
import json
...
class LicensePlateDataset(Dataset):
    ...
    def create_char_dict(self):
        ...
        char_to_idx = {char: idx + 1 for idx, char in enumerate(char_list)}  # Іindeksacіben 1 yaşındayım
        char_to_idx['<blank>'] = 0  # CTC-boşіsembol
        return char_to_idx
    ...
    def __getitem__(self, idx):
        image = Image.open(self.image_paths[idx]).convert('RGB')  # DönüştürmekіRGB'deyim
        if self.transform:
...
class OCRModel(pl.LightningModule):
    def __init__(self, num_classes):
        ...
        # Projeіİmzalıyorum
        self.fc = nn.Linear(backbone_c*backbone_h, 512)  # AyarlayınіdpovіCNN çıkışından önce alt

        # Pozisyonіyne koduvannya
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=500):
        ...
        pe[:, 0, 0::2] = torch.sin(position * div_term)  # Parnі іindeks
        pe[:, 0, 1::2] = torch.cos(position * div_term)  # Neparnі іindeks
...
def test(model, image_paths, labels, transform, idx_to_char):
    ...
    for image_path in image_paths:
        ...
        image = image.unsqueeze(0)  # Biberiye ekleіp paketi
        with torch.no_grad():
            logits = model(image)
...
if __name__ == '__main__':
    ...
    # Diyelim ki sende ne var є rota listeleriіresimden önceіdpovіgün mіakım
