...
from plaka_motoru.tools.via import VIADataset
from plaka_motoru.pipes.number_plate_keypoints_detectors.bbox_np_points_tools import normalize_rect_new
from plaka_motoru.pipes.number_plate_classificators.orientation_detector import OrientationDetector
from .image_processing import reshape_points

from plaka_motoru.tools.image_processing import (fline,
                                                 distance,
                                                 linear_line_matrix,
                                                 find_distances,
                                                 fix_clockwise2,
...
def check_blurriness(image: np.ndarray, threshold=5.0) -> bool:
    # Görüntüleri geçişlere dönüştürmeії сіbayatlamış rogoі віdkіgurur vericiі kanalів
    if len(image.shape) == 3:  # 3 kanallar (RGB)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    elif len(image.shape) == 4:  # 4 kanallar (RGB + Alfa)
        # ІGnoruєmo alfa kanalı і mektupєayіDerecelendirmelerdeki RGB renkleriії сіrogo
        image = cv2.cvtColor(image[:, :, :3], cv2.COLOR_BGR2GRAY)

    # sayacağımєMo Laplaceіbir (arkadaşın umrunda değilіalt) resim
    laplacian_var = cv2.Laplacian(image, cv2.CV_64F).var()

    # Çeviriіряєmo, chi menshe varіаціben varımіeşikte
