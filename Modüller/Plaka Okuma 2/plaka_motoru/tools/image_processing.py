...
def fline(p0: List, p1: List, debug: bool = False) -> List:
    """
    2 nokta kullanarak düz bir çizginin eğim açısının hesaplanması
    """
    x1 = float(p0[0])
    y1 = float(p0[1])
    ...
    if debug:
        print("Bu noktalardan geçen doğrunun denklemi:")
    if x1 - x2 == 0:
        k = math.inf
        b = y2
    else:
...
def linear_line_matrix(p0: List, p1: List, verbode: bool = False) -> np.ndarray:
    """
    İki noktayı kullanarak bir doğruyu tanımlayan bir matrisin katsayılarının hesaplanması
    """
    x1 = float(p0[0])
    y1 = float(p0[1])
...
def order_points_old(pts: np.ndarray):
    ...
    # Diyelim ki 3 noktamız var: A(x1,y1), B(x2,y2), C(x3,y3).
    # A ve B noktalarından düz bir çizgi çizilir. Ve C noktasının AB düz çizgisine göre nasıl konumlandığını belirlememiz gerekir.
    # Bunu yapmak için değeri hesaplıyoruz:
    # D = (x3 - x1) * (y2 - y1) - (y3 - y1) * (x2 - x1)
    # - Eğer D = 0 - Bu, C noktasının AB düz çizgisi üzerinde olduğu anlamına gelir.
    # - Eğer D < 0 - Bu, C noktasının doğrunun solunda olduğu anlamına gelir.
    # - Eğer D > 0 - Bu, C noktasının doğrunun sağında olduğu anlamına gelir.
    x1 = rect[0][0]
    y1 = rect[0][1]
    x2 = rect[2][0]
    y2 = rect[2][1]
...
def normalize_img(img: np.ndarray,
    ...
    if with_aug:
        from plaka_motoru.tools.augmentations import aug
        imgs = aug([img])
        img = imgs[0]
